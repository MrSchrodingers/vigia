...
async def main_async():
    parser = argparse.ArgumentParser(description="Reanalisa uma conversa do WhatsApp do banco de dados.")
    ...
    try:
        ...
        if not final_report:
            logging.warning(f"A reanálise para a conversa {args.conversa} não produziu um relatório. Verifique os logs do orquestrador.")
            return
        ...
        if args.salvar:
            ...
            try:
                database_service.save_whatsapp_analysis_results(db=db, conversation_jid=args.conversa, analysis_data=final_report)
        ...
        logging.error(f"Ocorreu um erro fatal durante a reanálise da conversa {args.conversa}: {e}", exc_info=True)
