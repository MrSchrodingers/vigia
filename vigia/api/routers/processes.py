...
from vigia.departments.negotiation_email.agents.run_ai_jury_pipeline import run_ai_jury_pipeline
from vigia.departments.negotiation_email.services.discord_notifier import send_discord_notification
from vigia.departments.negotiation_email.services.run_transit_analysis_for_process import run_transit_analysis_for_process
from vigia.services import crud
from vigia.services.jusbr_service import jusbr_service
from db.models import User, ProcessDocument, LegalProcess, TransitAnalysis
...
async def sync_and_get_process_details(
    ...
    for process_data in jusbr_data_list:
        process_in_db = crud.upsert_process_from_jusbr_data(db, process_data, user_id=current_user.id)
...
async def run_ai_jury_action(
    process_id: str,
    refresh: bool = Query(False, description="Se true, sincroniza com Jus.br antes de analisar"),
    db: Session = Depends(dependencies.get_db),
    current_user: User = Depends(dependencies.get_current_user),
):
    proc = db.query(LegalProcess)\
...
async def run_ai_jury_under_processes(
    ...
    return await run_ai_jury_action(process_id=process_id, refresh=refresh, db=db, current_user=current_user)
...
def create_transit_embed(process_number: str, analysis_result: dict):
    ...
    """
    justificativa = analysis_result.get("justificativa") or "Justificativa não informada."
    data_transit = analysis_result.get("data_transito_julgado") or "Não informada."

    justificativa_curta = (justificativa[:1021] + '...') if len(justificativa) > 1024 else justificativa
    ...
    embed = {
        ...
        "color": 3066993,
        "fields": [
            {"name": "Data do Trânsito em Julgado", "value": data_transit, "inline": True},
            {"name": "Justificativa da IA", "value": justificativa_curta, "inline": False}
        ],
...
@actions_router.post("/{process_id}/run-transit-analysis")
async def run_transit_analysis_action(
    ...
    """
    Executa a análise de IA para detectar o trânsito em julgado de um processo.
    """
    # 1. Valida se o processo existe e pertence ao usuário
    proc = db.query(LegalProcess)\
        .filter(LegalProcess.id == process_id, LegalProcess.owner_id == current_user.id)\
        .first()
    if not proc:
    ...
    # 2. Chama a função orquestradora que você já criou
    analysis_result = await run_transit_analysis_for_process(process_id=process_id, db=db)

    if "erro" in analysis_result:
        raise HTTPException(status_code=500, detail=analysis_result["erro"])

    # 3. Verifica o resultado e envia notificação se necessário
    if analysis_result.get("status_transito_julgado") == "Confirmado":
    ...
    # 4. Retorna o resultado da análise para o frontend
...
def read_transit_analyses(
    ...
    start_date: date | None= Query(None, description="Data inicial do trânsito (formato: AAAA-MM-DD)"),
    end_date: date | None= Query(None, description="Data final do trânsito (formato: AAAA-MM-DD)"),
    skip: int = 0,
    limit: int = 100,
):
    """
    ...
    analyses = query.order_by(TransitAnalysis.transit_date.desc()).offset(skip).limit(limit).all()
